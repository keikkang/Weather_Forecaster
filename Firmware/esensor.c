...
// Driver: BME680 (Environment sensor)
...
void ESENS_Get_Calib_Data(void)
{
	...
	bme680.calib.par_t1 = (uint16_t)((u8Data[1] << 8) | u8Data[0]);
	
	ESENS_Read(0x8A, &u8Data[0]);
	ESENS_Read(0x8B, &u8Data[1]);
	bme680.calib.par_t2 = (int16_t)((u8Data[1] << 8) | u8Data[0]);
	
	ESENS_Read(0x8C, &u8Data[0]);
	bme680.calib.par_t3 = (int8_t)(u8Data[0]);
	
	/* Pressure related coefficients */
	ESENS_Read(0x8E, &u8Data[0]);
	ESENS_Read(0x8F, &u8Data[1]);
	bme680.calib.par_p1 = (uint16_t)((u8Data[1] << 8) | u8Data[0]);
	
	ESENS_Read(0x90, &u8Data[0]);
	ESENS_Read(0x91, &u8Data[1]);
	bme680.calib.par_p2 = (int16_t)((u8Data[1] << 8) | u8Data[0]);
	
	ESENS_Read(0x92, &u8Data[0]);
	bme680.calib.par_p3 = (int8_t)(u8Data[0]);
	
	ESENS_Read(0x94, &u8Data[0]);
	ESENS_Read(0x95, &u8Data[1]);
	bme680.calib.par_p4 = (int16_t)((u8Data[1] << 8) | u8Data[0]);
	
	ESENS_Read(0x96, &u8Data[0]);
	ESENS_Read(0x97, &u8Data[1]);
	bme680.calib.par_p5 = (int16_t)((u8Data[1] << 8) | u8Data[0]);
	
	ESENS_Read(0x99, &u8Data[0]);
	bme680.calib.par_p6 = (int8_t)(u8Data[0]);
	
	ESENS_Read(0x98, &u8Data[0]);
	bme680.calib.par_p7 = (int8_t)(u8Data[0]);
	
	ESENS_Read(0x9C, &u8Data[0]);
	ESENS_Read(0x9D, &u8Data[1]);
	bme680.calib.par_p8 = (int16_t)((u8Data[1] << 8) | u8Data[0]);
	
	ESENS_Read(0x9E, &u8Data[0]);
	ESENS_Read(0x9F, &u8Data[1]);
	bme680.calib.par_p9 = (int16_t)((u8Data[1] << 8) | u8Data[0]);
	
	ESENS_Read(0xA0, &u8Data[0]);
	bme680.calib.par_p10 = (uint8_t)(u8Data[0]);
	
	/* Humidity related coefficients */
	ESENS_Read(0xE2, &u8Data[0]);
	ESENS_Read(0xE3, &u8Data[1]);
	bme680.calib.par_h1 = (uint16_t)((u8Data[1] << 4) | (u8Data[0] & 0x0F));
	
	ESENS_Read(0xE2, &u8Data[0]);
	ESENS_Read(0xE1, &u8Data[1]);
	bme680.calib.par_h2 = (uint16_t)((u8Data[1] << 4) | (u8Data[0] >> 4));
	
	ESENS_Read(0xE4, &u8Data[0]);
	bme680.calib.par_h3 = (int8_t)(u8Data[0]);
		
	ESENS_Read(0xE5, &u8Data[0]);
	bme680.calib.par_h4 = (int8_t)(u8Data[0]);
		
	ESENS_Read(0xE6, &u8Data[0]);
	bme680.calib.par_h5 = (int8_t)(u8Data[0]);
		
	ESENS_Read(0xE7, &u8Data[0]);	
	bme680.calib.par_h6 = (uint8_t)(u8Data[0]);
		
	ESENS_Read(0xE8, &u8Data[0]);
	bme680.calib.par_h7 = (int8_t)(u8Data[0]);
	
	/* Gas heater related coefficients */
	ESENS_Read(0xED, &u8Data[0]);
	bme680.calib.par_gh1 = (int8_t)(u8Data[0]);
	
	ESENS_Read(0xEB, &u8Data[0]);
	ESENS_Read(0xEC, &u8Data[1]);
	bme680.calib.par_gh2 = (int16_t)((u8Data[1] << 8) | u8Data[0]);
	
	ESENS_Read(0xEE, &u8Data[0]);
	bme680.calib.par_gh3 = (int8_t)(u8Data[0]);

	/* Other coefficients */
	ESENS_Read(0x02, &u8Data[0]);
	bme680.calib.res_heat_range = ((u8Data[0] & 0x30) / 16);
	
	ESENS_Read(0x00, &u8Data[0]);
	bme680.calib.res_heat_val = (int8_t)(u8Data[0]);

	ESENS_Read(0x04, &u8Data[0]);
	bme680.calib.range_sw_err = ((int8_t)u8Data[0] & (int8_t)0xf0) / 16;
}
...
float ESENS_Calc_Heater_Res(uint16_t temp)
{
	...
	var1 = (((float)bme680.calib.par_gh1 / (16.0f)) + 49.0f);
	var2 = ((((float)bme680.calib.par_gh2 / (32768.0f)) * (0.0005f)) + 0.00235f);
	var3 = ((float)bme680.calib.par_gh3 / (1024.0f));
	var4 = (var1 * (1.0f + (var2 * (float)temp)));
	var5 = (var4 + (var3 * (float)bme680.amb_temp));
	res_heat = (uint8_t)(3.4f * ((var5 * (4 / (4 + (float)bme680.calib.res_heat_range)) *
		(1/(1 + ((float) bme680.calib.res_heat_val * 0.002f)))) - 25));
...
void ESENS_Set_Gas_Config(void)
{
	...
	u8Data = (uint8_t)ESENS_Calc_Heater_Res(bme680.gas_sett.heatr_temp);
	ESENS_Write(0x5a, u8Data);
	
	u8Data = ESENS_Calc_Heater_Dur(bme680.gas_sett.heatr_dur);
	ESENS_Write(0x64, u8Data);

	bme680.gas_sett.nb_conv = 0;
}
...
void ESENS_Set_Oversampling(void)
{
	...
	u8Data &= ~(0x07);
	u8Data |= bme680.tph_sett.os_hum;
	ESENS_Write(0x72, u8Data);
	
	// temperature oversampling
	ESENS_Read(0x74, &u8Data);
	u8Data &= ~(0xE0);
	u8Data |= (bme680.tph_sett.os_temp << 5);
	ESENS_Write(0x74, u8Data);
	
	// pressure oversampling
	ESENS_Read(0x74, &u8Data);
	u8Data &= ~(0x1C);
	u8Data |= (bme680.tph_sett.os_pres << 2);
	ESENS_Write(0x74, u8Data);
}
...
void ESENS_Select_Filter(void)
{
	...
	u8Data &= ~(0x1C);
	u8Data |= bme680.tph_sett.filter << 2;
...
void ESENS_Select_RunGas(void)
{
	...
	u8Data &= ~(0x10);
	u8Data |= bme680.gas_sett.run_gas << 4;
...
void ESENS_Select_NBConv(void)
{
	...
	ESENS_Read(0x71, &u8Data);
	u8Data &= ~(0x0F);
	u8Data |= bme680.gas_sett.nb_conv;
	ESENS_Write(0x71, u8Data);
}
...
int16_t ESENS_Calc_Temperature(uint32_t temp_adc)
{
	...
	var1 = ((int32_t) temp_adc >> 3) - ((int32_t) bme680.calib.par_t1 << 1);
	var2 = (var1 * (int32_t) bme680.calib.par_t2) >> 11;
	var3 = ((var1 >> 1) * (var1 >> 1)) >> 12;
	var3 = ((var3) * ((int32_t) bme680.calib.par_t3 << 4)) >> 14;
	bme680.calib.t_fine = (int32_t) (var2 + var3);
	calc_temp = (int16_t) (((bme680.calib.t_fine * 5) + 128) >> 8);
...
uint32_t ESENS_Calc_Pressure(uint32_t pres_adc)
{
	...
	var1 = (((int32_t)bme680.calib.t_fine) >> 1) - 64000;
	var2 = ((((var1 >> 2) * (var1 >> 2)) >> 11) *
		(int32_t)bme680.calib.par_p6) >> 2;
	var2 = var2 + ((var1 * (int32_t)bme680.calib.par_p5) << 1);
	var2 = (var2 >> 2) + ((int32_t)bme680.calib.par_p4 << 16);
	var1 = (((((var1 >> 2) * (var1 >> 2)) >> 13) *
		((int32_t)bme680.calib.par_p3 << 5)) >> 3) +
	...
	var1 = ((int32_t)bme680.calib.par_p9 * (int32_t)(((pressure_comp >> 3) *
		(pressure_comp >> 3)) >> 13)) >> 12;
	var2 = ((int32_t)(pressure_comp >> 2) *
		(int32_t)bme680.calib.par_p8) >> 13;
	var3 = ((int32_t)(pressure_comp >> 8) * (int32_t)(pressure_comp >> 8) *
		(int32_t)(pressure_comp >> 8) *
		(int32_t)bme680.calib.par_p10) >> 17;

	pressure_comp = (int32_t)(pressure_comp) + ((var1 + var2 + var3 +
		((int32_t)bme680.calib.par_p7 << 7)) >> 4);
...
uint32_t ESENS_Calc_Humidity(uint16_t hum_adc)
{
	...
	temp_scaled = (((int32_t) bme680.calib.t_fine * 5) + 128) >> 8;
	var1 = (int32_t) (hum_adc - ((int32_t) ((int32_t) bme680.calib.par_h1 * 16)))
		- (((temp_scaled * (int32_t) bme680.calib.par_h3) / ((int32_t) 100)) >> 1);
	var2 = ((int32_t) bme680.calib.par_h2
		* (((temp_scaled * (int32_t) bme680.calib.par_h4) / ((int32_t) 100))
			+ (((temp_scaled * ((temp_scaled * (int32_t) bme680.calib.par_h5) / ((int32_t) 100))) >> 6)
				/ ((int32_t) 100)) + (int32_t) (1 << 14))) >> 10;
	var3 = var1 * var2;
	var4 = (int32_t) bme680.calib.par_h6 << 7;
	var4 = ((var4) + ((temp_scaled * (int32_t) bme680.calib.par_h7) / ((int32_t) 100))) >> 4;
...
uint32_t ESENS_Calc_Gas_Resistance(uint16_t gas_res_adc, uint8_t gas_range)
{
	...
	var1 = (int64_t) ((1340 + (5 * (int64_t)bme680.calib.range_sw_err)) *
...
void ESENS_Read_Field_Data(void)
{
	...
	bme680_data.status = u8Data[0] & 0x80;
	bme680_data.gas_index = u8Data[0] & 0x0F;
	
	ESENS_Read(0x1E, &u8Data[0]);
	bme680_data.meas_index = u8Data[0];
	...
	bme680_data.status |= u8Data[0] & 0x20;
	bme680_data.status |= u8Data[0] & 0x10;
	
	if (bme680_data.status & 0x80)
	{
		bme680_data.temperature = ESENS_Calc_Temperature(adc_temp);
		bme680_data.pressure = ESENS_Calc_Pressure(adc_pres);
		bme680_data.humidity = ESENS_Calc_Humidity(adc_hum);
		bme680_data.gas_resistance = ESENS_Calc_Gas_Resistance(adc_gas_res, gas_range);
	}
}
...
int16_t ESENS_Get_Humidity_Score(void)
{
  ...
  float current_humidity = bme680_data.humidity / 1000.0f;
...
{
	int16_t gas_score;
	float gas_reference = bme680_data.gas_resistance;
...
void ESENS_Measure_Init(void)
{
	...
	bme680.tph_sett.os_hum = 2;
	bme680.tph_sett.os_temp = 4;
	bme680.tph_sett.os_pres = 3;
	ESENS_Set_Oversampling();
	
	bme680.tph_sett.filter = 2;
	ESENS_Select_Filter();
}
...
void ESENS_Measure(void)
{
	...
	bme680.gas_sett.run_gas = 1;
	ESENS_Select_RunGas();
	
	bme680.gas_sett.heatr_temp = 350;
	...
	//printf("Temperature = %.2f C\n", bme680_data.temperature / 100.0f);
	bme680.amb_temp = (int8_t)(bme680_data.temperature / 100.0f); // update amb temp for gas measure
  //printf("Pressure = %.2f hPa\n", bme680_data.pressure / 100.0f);
  //printf("Humidity = %.2f %%\n", bme680_data.humidity / 1000.0f);
	/* Avoid using measurements from an unstable heating setup */
  //if(bme680_data.status & 0x20)
  ...
  bme680_data.air_quality_score = humidity_score + gas_score;
	//printf("Score = %d\n", bme680_data.air_quality_score);
	//printf("Score = %s\n", ESENS_CalculateIAQ(bme680_data.air_quality_score));
}
...
{
	return bme680_data.air_quality_score;
}

#ifdef ESENS_TEST
void ESENS_Test(void)
{
  ...
  air_quality_score = humidity_score + gas_score;
	printf("Score = %d\n", air_quality_score);
	printf("Score = %s\n", ESENS_CalculateIAQ(air_quality_score));
}
#endif
