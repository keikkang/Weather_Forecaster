...
#define WIFIM_PU								(PE12 = 1)
#define WIFIM_PD								(PE12 = 0)
...
{
	memset(&g_u8WifiRxBuf[0], 0, sizeof(g_u8WifiRxBuf));
	g_s16WifiRxHead  = 0;
	g_s16WifiRxTail  = 0;
}
...
uint8_t WIFIM_CheckReady(void)
{
	...
	while (!u8Done)
	{
		if (WIFIM_QueueSize() > 0)
		{
			...
			UART_Write(UART0, &u8Buffer, 1); // for debug
		...
		u32Timeout++;
		vTaskDelay( ( 1 / ( ( TickType_t ) 1000000 / configTICK_RATE_HZ ) ) ); // 1 ms delay
		if (u32Timeout > 200000)
		{
			return 1;
			//u8Done = 1;
		}
